...
function renderSection(i){
  ...
  buildToc(); window.scrollTo(0,0); store('ch0904.sec',String(i));
}
function buildToc(){
  let g=null, h='';
...
ltog.onclick=()=>{ const o=lbar.classList.toggle('open'); ltog.setAttribute('aria-expanded',o?'true':'false'); store('ch0904.leg',o?'1':'0'); };
if(store('ch0904.leg')==='1'){ lbar.classList.add('open'); ltog.setAttribute('aria-expanded','true'); }
...
function applyTheme(t){
  ...
  store('ch0904.theme',t||'auto');
}
let theme=store('ch0904.theme')||'auto'; applyTheme(theme);
tbtn.onclick=()=>{ theme = theme==='auto'?'light':(theme==='light'?'dark':'auto'); applyTheme(theme); renderSection(idx); };

const saved=parseInt(store('ch0904.sec')||'0',10);
